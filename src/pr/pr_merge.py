...
def merge_github_pr(config, gh_token, file_path, pr_body=""):
    ...
    # 优先使用配置中指定的合并文件路径
    if config['github'].get('merge_file_path'):
        repo_file_path = config['github']['merge_file_path']
    # 其次使用配置中指定的GitHub文件路径
    elif config['github'].get('file_path'):
    ...
    if response.status_code == 200:
        # If file exists, add new content before existing content
        existing_content = base64.b64decode(response.json()['content']).decode('utf-8')
        merged_content = new_content + '\n' + existing_content
        file_data['sha'] = response.json()['sha']
    else:
        # If file doesn't exist, use only the new content
        merged_content = new_content
    
    # Encode the merged content
    file_data['content'] = base64.b64encode(merged_content.encode('utf-8')).decode('utf-8')
