...
- [Dichotomous Diffusion Policy Optimization](https://arxiv.org/abs/2601.00898)
  ...
  - Datasets: [ExORL](https://github.com/rail-berkeley/rlpd), [OGBench](https://github.com/OpenGVLab/ogbench), [NAVSIM](https://github.com/autonomousvision/navsim)
  - Summary：
    - Proposes DIPOLE, a novel RL algorithm for stable and controllable diffusion policy optimization, decomposing the optimal policy into dichotomous policies for reward maximization and minimization.
    ...
    - Demonstrates effectiveness in offline and offline-to-online RL settings and trains a VLA model for end-to-end autonomous driving, evaluated on NAVSIM.
...
- [Counterfactual VLA: Self-Reflective Vision-Language-Action Model with Adaptive Reasoning](https://arxiv.org/abs/2512.24426)
  ...
  - Publisher: Stanford University, NVIDIA, MIT, University of California, Los Angeles, University of Michigan, Tsinghua University
  ...
  - Summary：
    ...
    - Demonstrates improvements in trajectory accuracy (up to 17.6%) and safety metrics (20.5%), with adaptive counterfactual reasoning activated primarily in challenging scenarios.
...
- [Vision-Language-Action Models for Autonomous Driving: Past, Present, and Future](https://arxiv.org/abs/2512.16760)
  ...
  - Task: End-to-End
  - Summary：
    - Provides a structured characterization of the emerging Vision-Language-Action (VLA) landscape for autonomous driving, tracing its evolution from Vision-Action models.
    - Organizes existing VLA methods into two principal paradigms: End-to-End VLA and Dual-System VLA, with further sub-classifications.
    - Summarizes representative datasets and benchmarks, and highlights key challenges including robustness, interpretability, and instruction fidelity.
...
- [MindDrive: A Vision-Language-Action Model for Autonomous Driving via Online Reinforcement Learning](https://arxiv.org/abs/2512.13636)
  ...
  - Summary：
    ...
    - The framework employs a single LLM with two LoRA parameter sets: a Decision Expert for reasoning and an Action Expert to map decisions to trajectories, enabling efficient trial-and-error learning over discrete linguistic decisions.
    - Achieves a Driving Score of 78.04 and Success Rate of 55.09% on the Bench2Drive benchmark using the lightweight Qwen-0.5B LLM, demonstrating the first effective use of online RL for VLA in autonomous driving.
...
- [DrivePI: Spatial-aware 4D MLLM for Unified Autonomous Driving Understanding, Perception, Prediction and Planning](https://arxiv.org/abs/2512.12799)
  ...
  - Datasets: [nuScenes](https://www.nuscenes.org/)
  - Summary：
    - DrivePI, a spatial-aware 4D MLLM serving as a unified Vision-Language-Action (VLA) framework that jointly performs spatial understanding, 3D perception, prediction, and planning through end-to-end optimization.
    - The approach integrates point clouds, multi-view images, and language instructions, and uses a data engine to generate text-occupancy and text-flow QA pairs for 4D spatial understanding.
    - With only a 0.5B model backbone, DrivePI as a single unified model matches or exceeds both existing VLA models and specialized vision-action models across multiple benchmarks for perception, prediction, and planning.
...
- [Latent Chain-of-Thought World Modeling for End-to-End Driving](https://arxiv.org/abs/2512.10226)
  ...
  - Publisher: Stanford University, University of Texas at Austin, NVIDIA, University of California, Berkeley
  - Publish Date: 2025.12.11
  - Task: End-to-End
  - Summary：
    - Introduces Latent-CoT-Drive (LCDrive), a model that performs chain-of-thought reasoning in a learned latent language aligned with action tokens, rather than natural language.
    - Unifies reasoning and decision-making by interleaving action-proposal tokens and world model tokens grounded in a latent world model to predict future outcomes.
    - Achieves improved inference speed, trajectory quality, and reinforcement learning gains on end-to-end driving benchmarks compared to non-reasoning and text-reasoning baselines.
...
- [UniUGP: Unifying Understanding, Generation, and Planing For End-to-end Autonomous Driving](https://arxiv.org/abs/2512.09864)
  ...
  - Summary：
    ...
    - Introduces a four-stage training strategy across multiple AD datasets and proposed specialized datasets, demonstrating state-of-the-art performance and superior generalization to long-tail scenarios.
...
- [WAM-Diff: A Masked Diffusion VLA Framework with MoE and Online Reinforcement Learning for Autonomous Driving](https://arxiv.org/abs/2512.11872)
  ...
  - Summary：
    - WAM-Diff, a Vision-Language-Action (VLA) framework that employs masked diffusion to iteratively refine discrete sequences for future ego-trajectory generation in autonomous driving.
    - Introduces a sparse Mixture of Experts (MoE) architecture trained jointly on motion prediction and driving-oriented VQA, and uses online reinforcement learning (Group Sequence Policy Optimization) to optimize sequence-level driving rewards.
...
- [WAM-Flow: Parallel Coarse-to-Fine Motion Planning via Discrete Flow Matching for Autonomous Driving](https://arxiv.org/abs/2512.06112)
  ...
  - Summary：
    ...
    - The approach integrates a metric-aligned numerical tokenizer, a geometry-aware flow objective, and simulator-guided GRPO alignment for safety, progress, and comfort, while converting a pre-trained auto-regressive backbone into a non-causal flow model.
    - The model achieves superior closed-loop performance, with 1-step inference at 89.1 PDMS and 5-step at 90.3 PDMS on the NAVSIM v1 benchmark, establishing discrete flow matching as a promising paradigm for end-to-end autonomous driving.
